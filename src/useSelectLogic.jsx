...
export const makeId = (str, fallback = 'invalid-option') => {
...
export function useSelectLogic({
    ...
    emptyholder = 'No options',
    disabledholder = 'Disabled',
    loadingholder = 'Loading',
    errorholder = 'Failed to load',
    ...
    ), [error, loading, disabled, options])
    ...
    const normalizedPropOptions = useMemo(() => {
        ...
        return flat.map((item, i) => {
            const baseId = makeId(item.key ?? item.label ?? i)
            return {
    ...
    return {normalizedOptions, selected, selectOption, clear, hasOptions, active, selectedValue, placeholder, emptyholder, disabledholder, loadingholder, errorholder, disabledoption, emptyoption, invalidoption, disabled, loading, error}
}